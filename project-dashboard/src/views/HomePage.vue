<template>
  <el-container style="min-height: 100vh; padding-top: 100px;">
    <!-- 项目总览标题栏 -->
    <div style="position: fixed; top: 0; left: 0; right: 0; z-index: 1000; background: linear-gradient(135deg, #409EFF 0%, #4d9eff 100%); padding: 20px; box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);">
      <div style="max-width: 1200px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="text-align: center; flex: 1;">
          <h1 style="margin: 0; font-size: 28px; color: white; font-weight: bold; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3);">工艺技术部 | 新产品项目总览</h1>
        ...
        <!-- 项目分类统计 -->
          <el-card shadow="hover" margin-bottom="20px" style="margin-top: 20px;">
            ...
            <el-row :gutter="20">
              <el-col :span="6">
                <el-card 
                  shadow="hover" 
                  @click="goToProjectDetail('total')" 
                  class="clickable-card stats-card"
                  style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);"
                >
                  <div class="card-title" style="color: white;">项目总数</div>
                  <div class="card-value" style="color: white; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">{{ projectCategoryStats.total_projects || 0 }}个</div>
                </el-card>
              </el-col>
              <el-col :span="6">
                <el-card 
                  shadow="hover" 
                  @click="goToProjectDetail('not_started')" 
                  class="clickable-card stats-card"
                  style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);"
                >
                  <div class="card-title" style="color: white;">未开始项目</div>
                  <div class="card-value" style="color: white; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">{{ projectCategoryStats.not_started_projects || 0 }}个</div>
                </el-card>
              </el-col>
              <el-col :span="6">
                <el-card 
              ...
              <el-col :span="6">
                <el-card 
                  shadow="hover" 
                  @click="goToProjectDetail('completed')" 
                  class="clickable-card stats-card"
                  style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);"
                >
                  <div class="card-title" style="color: white;">已结项项目</div>
                  <div class="card-value" style="color: white; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">{{ projectCategoryStats.completed_projects || 0 }}个</div>
                </el-card>
              </el-col>
            </el-row>
          </el-card>
          ...
          <el-row :gutter="20" margin-bottom="20px">
            ...
            <el-col :span="8">
              <el-card 
                shadow="hover" 
                class="clickable-card chart-card"
                style="background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%);"
              >
                <div slot="header" class="card-header" style="color: #2c3e50;">项目状态分布</div>
            ...
            <el-col :span="8">
              ...
              >
                <div slot="header" class="card-header" style="color: #2c3e50;">异常子任务负责人统计</div>
                <el-table
            ...
            <el-col :span="8">
              ...
              >
                <div slot="header" class="card-header" style="color: #2c3e50;">项目经理负载</div>
              ...
              <el-table-column prop="category" label="项目分类" width="120" align="center" header-align="center">
                <template #default="scope">
                  <div class="category-display">
                    <!-- 进度条样式 -->
                    <div class="category-progress-container" v-if="scope.row.category">
                      <div 
                        class="category-progress-bar" 
                        :class="getCategoryClass(scope.row.category)"
                        :style="getCategoryProgressStyle(scope.row.category)"
                      >
...
// 解构 ElLoading 服务
const { ElLoadingService } = ElLoading || {};

// Element Plus 服务组件安全调用包装函数
const safeElLoading = (options) => {
  try {
    if (ElLoading && typeof ElLoading.service === 'function') {
      return ElLoading.service(options);
    } else {
      console.warn('ElLoading 服务不可用，使用备用方案');
      // 返回一个模拟的loading对象
      return {
    ...
    console.warn('ElLoading 调用失败:', error);
    return {
      close: () => console.log('Loading closed')
    };
  }
...
const safeElMessage = (message, options = {}) => {
  try {
    if (ElMessage && typeof ElMessage === 'function') {
      ...
      console.warn('ElMessage 服务不可用，使用console输出');
...
const getCategoryProgressStyle = (category) => {
  ...
  switch (category) {
    case '未开始':
      return {
        ...baseStyle,
        background: 'linear-gradient(90deg, #FFEAA7 0%, #FDCA40 100%)',
        color: '#2D3436',
        boxShadow: '0 2px 8px rgba(253, 202, 64, 0.3)'
      }
    case '进行中':
      return {
        ...baseStyle,
    ...
    default:
      return {
        ...baseStyle,
        background: 'linear-gradient(90deg, #DDDDDD 0%, #BBBBBB 100%)',
        color: '#666666',
      ...
      if (response && response.needs_confirmation) {
        try {
          ...
          if (overwriteConfirm === 'confirm') {
            ...
            const overwriteLoading = safeElLoading({
              lock: true,
              text: '正在覆盖数据...',
              background: 'rgba(0, 0, 0, 0.7)'
            });
...
const initTypePie = async () => {
  try {
    ...
    typePieChart.on('click', async function(params) {
      ...
      console.log('处理后的状态:', status);
      
      if (!status) {
        console.warn('⚠️ 未识别到点击的状态');
        ElMessage.warning('未识别到点击的状态');
        return;
      }

      try {
        console.log(`准备获取${status}项目数据`);
        ...
        try {
          loadingInstance = ElLoading.service({
            lock: true,
            text: `正在加载${status}项目数据...`,
            background: 'rgba(0, 0, 0, 0.7)'
          });
        } catch (e) {
          console.warn('加载提示创建失败，使用消息提示:', e);
          ElMessage.info(`正在加载${status}项目数据...`);
        }

        // 调用API接口获取对应状态的任务数据
        console.log('调用API: getProjectTasksByStatus');
        console.log('API参数:', status);
        
        const projectTasksData = await projectApi.getProjectTasksByStatus(status);
        console.log('✅ API调用成功');
        console.log('获取到的数据:', projectTasksData);
        console.log('数据条数:', projectTasksData?.length || 0);
        ...
        if (projectTasksData && projectTasksData.length > 0) {
          ...
          console.log('ℹ️ 没有获取到数据，将使用API直接获取');
        }
        ...
        console.log('状态参数转换:', { original: status, converted: statusParam });
        ...
        // 显示跳转提示
        ElMessage.success(`即将跳转到${status}项目详情页面`);
        ...
        ElMessage.error(`获取项目任务数据失败：${error.message}`);
      }
...
.category-not-started {
  background: linear-gradient(90deg, #FFEAA7 0%, #FDCA40 100%) !important;
}
...
.category-completed {
  background: linear-gradient(90deg, #55EFC4 0%, #00B894 100%) !important;
}

.category-default {
  background: linear-gradient(90deg, #DDDDDD 0%, #BBBBBB 100%) !important;
}
