<template>
  <div class="accepted-task-detail">
    ...
    <div class="stats-cards">
      <el-card class="stat-card">
        <div class="stat-content">
          <div class="stat-number">{{ acceptedTaskStats.totalAcceptedTasks }}</div>
          <div class="stat-label">已验收任务数</div>
        </div>
      </el-card>
      
      <el-card class="stat-card">
        <div class="stat-content">
          <div class="stat-number" :style="{ color: '#67C23A' }">{{ acceptedTaskStats.totalTasks }}</div>
      ...
      <el-card class="stat-card">
        <div class="stat-content">
          <div class="stat-number" :style="{ color: '#E6A23C' }">{{ acceptedTaskStats.pendingTasks }}</div>
          <div class="stat-label">待验收任务</div>
        </div>
      </el-card>
    </div>
...
const fetchAcceptedTasks = async () => {
  loading.value = true
  try {
    // 获取所有已验收任务（填写了实际开始和实际结束时间的任务）
    const response = await projectApi.getTasksByType({
      type: 'completed_task',  // 获取所有已验收任务
      page: currentPage.value,
      limit: pageSize.value
    })
    
    acceptedTasks.value = response || []
    filteredTasks.value = acceptedTasks.value
    totalTasks.value = await getAcceptedTasksCount()
    
    // 计算统计信息
    calculateStats()
  } catch (error) {
    console.error('获取已验收任务失败:', error)
  } finally {
    loading.value = false
  }
}
...
// 获取所有已验收任务数据
const fetchAllAcceptedTasks = async () => {
  try {
    // 获取所有已验收任务（使用大分页一次性获取）
    const response = await projectApi.getTasksByType({
      type: 'completed_task',
      page: 1,
    ...
    console.error('获取所有已验收任务失败:', error)
    return []
  }
}
...
const calculateStats = async () => {
  // 获取所有已验收任务和所有任务来准确计算统计信息
  const allAcceptedTasks = await fetchAllAcceptedTasks()
  const allTasks = await fetchAllTasks()
  
  const totalAccepted = allAcceptedTasks.length
  const totalTasks = allTasks.length
  const pending = totalTasks - totalAccepted
  
  acceptedTaskStats.value = {
    totalAcceptedTasks: totalAccepted,
    totalTasks: totalTasks,
    pendingTasks: pending
  }
  
  // 同时更新当前页面显示的数据
  acceptedTasks.value = allAcceptedTasks
  filteredTasks.value = allAcceptedTasks.slice(
    (currentPage.value - 1) * pageSize.value,
    currentPage.value * pageSize.value
  )
  totalTasks.value = allAcceptedTasks.length
}

// 获取已验收任务总数
const getAcceptedTasksCount = async () => {
  try {
    const count = await projectApi.getTasksByTypeCount('completed_task')
    return count || 0
  } catch (error) {
    console.error('获取已验收任务总数失败:', error)
    return 0
  }
}
...
// 处理搜索
const handleSearch = () => {
  let result = acceptedTasks.value
